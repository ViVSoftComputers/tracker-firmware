...
// 4-click gesture: toggle GPS on/off and broadcast position if enabling
void CachedPhoneTracker::toggleGPS()
{
    ...
    if (gps->isEnabled()) {
        gps->disable();
        playDirectTone(2000, 80);
        LOG_INFO("CachedPhoneTracker: GPS disabled via 4-click gesture\n");
    } else {
        gps->enable();
        playDirectTone(2400, 60);
        delay(40);
        playDirectTone(2800, 80);
        service->refreshLocalMeshNode();
        LOG_INFO("CachedPhoneTracker: GPS enabled + position broadcast via 4-click gesture\n");
    }
}

// 5-click gesture: respond with a ping acknowledgment
void CachedPhoneTracker::sendPing()
{
    if (cachedPhoneTracker) {
        cachedPhoneTracker->replyText("PING OK");
    }
    playDirectTone(3000, 40);
    LOG_INFO("CachedPhoneTracker: Ping acknowledged via 5-click gesture\n");
}
...
void CachedPhoneTracker::toggleTrackerMode()
{
    ...
    if (trackerModeActive) {
        ...
        if (cachedPhoneTracker) {
            cachedPhoneTracker->last_capture_ms = 0;
        }
